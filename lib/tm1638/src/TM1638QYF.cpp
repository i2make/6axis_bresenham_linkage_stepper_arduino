	...
	const byte numberFont[])
{
	...
	snprintf(values, displays + 1, leadingZeros ? "%08luX" : "%luX", number); // ignores display size
		...
		const byte numberFont[])
{
	...
	snprintf(values, displays + 1, leadingZeros ? "%08lu" : "%lu", number); // ignores display size
